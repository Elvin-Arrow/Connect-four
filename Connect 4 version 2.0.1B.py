    ...
    def gamewindow():   #The function for main game window
        ...
        def setVictoryBar():    #Function to display cross on the four connected pieces
            ...
            if winDirection == "Horizontal":
                ...
                #Find the least column number from the win list
                smallest = winList[0]   
                for i in range(1, 4):
                ...
                time.sleep(1)
                  ...
                  #Find the least row number from the win list
                  smallest = winList[0]
                  for i in range(1, 4):
                      if winList[i] < smallest:
                          smallest = winList[i]
                ...
                for i in range(5, largest_y+1, -1):
                    ycoords -= 100
                Label(game, image=cross, bd=0).place(x=xcoords, y=ycoords)
                game.update()
                time.sleep(1)
            else:           
                xcoords = 205
                ...
                for i in range(0, smallest_y, 1):
                    ycoords += 100
                cr = Label(game, image=cross, bd=0)
                cr.place(x=xcoords, y=ycoords)
                game.update()
                time.sleep(1)

        global resetFlag
        #if the game has not been rest then destroy the main window
        if not resetFlag:
            root.destroy()
